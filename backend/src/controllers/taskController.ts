    ...
    try {
        ...
        if (teamId) {
            ...
            if (!allowed) {
                res.status(403).json({
                    message:
                        'Only team owners and admins can create tasks.',
                });
                return;
            }
        }
        ...
        if (assignmentType === 'team' && team) {
            assigneeIds = team.members.map(
                (m) => m.user as Types.ObjectId,
            );
        } else if (assignedTo) {
            assigneeIds = Array.isArray(assignedTo)
                ? assignedTo
                : [assignedTo];
        } else {
            assigneeIds = [
                new Types.ObjectId(assignerId),
            ];
        }
        ...
        if (teamId && team) {
            ...
            const invalid = assigneeIds.filter(
                (id) =>
            ...
            if (invalid.length) {
                res.status(400).json({
                    message:
                        'Some assignees are not members of the team',
                });
                return;
            }
        }
        ...
        const notifRecipients = assigneeIds.filter(
            (id) => id.toString() !== assignerId,
        );

        console.log(
            '📩 notifRecipients:',
            notifRecipients.length,
            notifRecipients,
        );
        ...
        if (notifRecipients.length > 0) {
            try {
                console.log(
                    '📡 calling sendTaskAssignedNotification',
                );

                await sendTaskAssignedNotification(
                    task,
                    req.user,
                    notifRecipients.map(String),
                );
            } catch (e) {
                console.error(
            ...
            console.log(
                '⚠️ No recipients to notify — skipping FCM',
            );
        }
...
export const updateTask = async (
    ...
    try {
        ...
        const allowed = await canEditTask(
            userId,
            id,
        );
...
export const deleteTask = async (
    ...
    try {
        ...
        const allowed = await canDeleteTask(
            userId,
            id,
        );
        ...
        res.json({
            message:
                'Task deleted successfully and preserved in history',
        });
    } catch (err) {
