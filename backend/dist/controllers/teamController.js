...
exports.deleteTeam = exports.leaveTeam = exports.removeTeamMember = exports.updateTeamSettings = exports.getPendingInvitations = exports.respondToInvitation = exports.inviteToTeam = exports.getTeamDetails = exports.getUserTeams = exports.createTeam = void 0;
const Team_1 = __importDefault(require("../models/Team"));
const User_1 = __importDefault(require("../models/User"));
const TeamInvitation_1 = __importDefault(require("../models/TeamInvitation"));
...
const createTeam = async (req, res) => {
    try {
        ...
        if (name.trim().length > 100) {
            res.status(400).json({ message: 'Team name must be under 100 characters' });
            return;
        }
        ...
        await team.save();
        await User_1.default.findByIdAndUpdate(userId, { $addToSet: { teams: team._id } });
        await team.populate([
...
const getUserTeams = async (req, res) => {
    try {
        const userId = req.userId;
        const teams = await Team_1.default.find({ 'members.user': userId, isActive: true })
...
const inviteToTeam = async (req, res) => {
    try {
        ...
        if (!canInvite) {
            res.status(403).json({ message: 'You do not have permission to invite members' });
            return;
        }
        let invitee = null;
        if (inviteId) {
            invitee = await User_1.default.findOne({ inviteId, isPublic: true });
            if (!invitee) {
            ...
            invitee = await User_1.default.findOne({ email: email.toLowerCase().trim() });
            if (!invitee) {
            ...
            res.status(400).json({ message: 'Either email or invite ID is required' });
            return;
        }
        ...
        const existing = await TeamInvitation_1.default.findOne({ team: teamId, invitee: invitee._id });
        if (existing) {
            if (existing.status === 'pending') {
                res.status(400).json({ message: 'User already has a pending invitation to this team' });
                return;
            }
            if (existing.status === 'accepted') {
                res.status(400).json({ message: 'This invitation was already accepted' });
                return;
            }
            await TeamInvitation_1.default.deleteOne({ _id: existing._id });
        }
        const invitation = new TeamInvitation_1.default({
            team: teamId,
            inviter: inviterId,
            invitee: invitee._id,
            email: invitee.email,
        ...
        try {
            await (0, notificationService_1.sendNotification)(invitee._id.toString(), {
                title: 'Team Invitation',
            ...
            console.error('FCM error (non-critical):', e);
        }
...
const respondToInvitation = async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const invitation = await TeamInvitation_1.default.findById(invitationId)
            .populate('team')
        ...
        if (invitation.status !== 'pending') {
            res.status(400).json({ message: `Invitation already ${invitation.status}` });
            return;
        }
        ...
        if (response === 'accepted') {
            const team = await Team_1.default.findById(invitation.team._id);
            if (team) {
                team.members.push({ user: new mongoose_1.Types.ObjectId(userId), role: invitation.role, joinedAt: new Date(), invitedBy: invitation.inviter });
                await team.save();
                await User_1.default.findByIdAndUpdate(userId, { $push: { teams: team._id } });
                ...
                if (memberNotifs.length)
                    await Notification_1.default.insertMany(memberNotifs);
            }
        }
...
const getPendingInvitations = async (req, res) => {
    try {
        const userId = req.userId;
        const invitations = await TeamInvitation_1.default.find({
            invitee: userId,
            status: 'pending',
...
const updateTeamSettings = async (req, res) => {
    try {
        ...
        if (!member || !['owner', 'admin'].includes(member.role)) {
            res.status(403).json({ message: 'Only owners and admins can update settings' });
            return;
        }
...
const removeTeamMember = async (req, res) => {
    try {
        ...
        await team.save();
        await User_1.default.findByIdAndUpdate(memberId, { $pull: { teams: teamId } });
...
const leaveTeam = async (req, res) => {
    try {
        ...
        if (member.role === 'owner') {
            res.status(400).json({ message: 'Team owner cannot leave. Transfer ownership first.' });
            return;
        }
        team.members = team.members.filter((m) => m.user.toString() !== userId);
        await team.save();
        await User_1.default.findByIdAndUpdate(userId, { $pull: { teams: teamId } });
...
const deleteTeam = async (req, res) => {
    try {
        ...
        if (team.owner.toString() !== userId) {
            res.status(403).json({ message: 'Only team owner can delete the team' });
            return;
        }
        await User_1.default.updateMany({ teams: teamId }, { $pull: { teams: teamId } });
        team.isActive = false;
        await team.save();
